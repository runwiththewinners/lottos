...
function PaywallCard({ play }: { play: Play }) {
  const handleUpgrade = () => {
    window.open('https://whop.com/rwtw/rwtw/?utm_source=store_page&funnelId=store_063b60b3-5217-406f-90e5-0460512a85a2', '_blank');
  };
  const legCount = play.legs?.length || 0;
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, color: "#6b7280", fontFamily: "'Courier Prime', monospace", marginBottom: 12, lineHeight: 1.5 }}>
            Premium & High Roller members get all lotto picks
          </div>
...
function AdminPanel({ onPost, onClose }: { onPost: (play: any) => void; onClose: () => void }) {
  ...
  const [parlayOdds, setLottoOdds] = useState("");
    ...
    <div style={{
      ...
      <div style={{ padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
          <div>
            <label style={labelStyle}>Lotto Odds</label>
            <input style={inputStyle} placeholder="e.g. +450" value={parlayOdds} onChange={(e) => setLottoOdds(e.target.value)} />
          </div>
          <div>
      ...
      if (res.ok) {
        ...
        try {
          await fetch("/api/notify", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ team: playData.legs?.[0]?.team || "Lotto", odds: playData.parlayOdds, sport: playData.legs?.[0]?.sport || "NBA" }),
          });
        } catch {}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 14, fontFamily: "'Oswald', sans-serif", fontWeight: 700, color: "#f5f5f5" }}>
                {hasPremiumAccess || isAdmin
                  ? `${notification.legs?.length}-leg lotto (${notification.parlayOdds})`
                  : "New lotto dropped! Upgrade to view"}
              </div>
            </div>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "40px 20px 80px", position: "relative" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 36, animation: "fadeSlideIn 0.5s ease forwards" }}>
          ...
          <h1 style={{ fontSize: 48, fontFamily: "'Oswald', sans-serif", fontWeight: 800, lineHeight: 1, textTransform: "uppercase", marginBottom: 4 }}>
            <span style={{ color: "#f5f5f5" }}>Lotto</span><br />
          ...
          <p style={{ fontSize: 13, color: "#6b7280", maxWidth: 380, margin: "12px auto 0", lineHeight: 1.6, fontFamily: "'Courier Prime', monospace" }}>
            High-odds lotto plays, posted live.
          </p>
        </div>
        ...
        {/* Feed */}
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "#6b7280", fontSize: 13, fontFamily: "'Courier Prime', monospace" }}>Loading lottos...</div>
        ) : (
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {pendingPlays.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 20px", color: "#4b5563", fontSize: 13, fontFamily: "'Courier Prime', monospace" }}>No lottos yet. Check back soon.</div>
              ) : hasPremiumAccess || isAdmin ? (
                pendingPlays.map(play => <LottoCard key={play.id} play={play} onUpdateResult={handleUpdateResult} onDelete={handleDelete} isAdmin={isAdmin} />)
              ) : (
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#6b7280', fontFamily: "'Courier Prime', monospace", lineHeight: 1.6, marginBottom: 8 }}>
                    {pendingPlays.length} active lotto{pendingPlays.length !== 1 ? 's' : ''} available
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 280, margin: '0 auto' }}>
                    <button onClick={() => window.open('https://whop.com/rwtw/rwtw/?utm_source=store_page&funnelId=store_063b60b3-5217-406f-90e5-0460512a85a2', '_blank')} style={{
                    ...
                    <button onClick={() => window.open('https://whop.com/rwtw/rwtw-premium-copy/?utm_source=store_page&funnelId=store_063b60b3-5217-406f-90e5-0460512a85a2', '_blank')} style={{
              ...
              <div style={{ marginTop: 36 }}>
                ...
                {showGraded && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 16 }}>
                    {gradedPlays.map(play => <LottoCard key={play.id} play={play} onUpdateResult={handleUpdateResult} onDelete={handleDelete} isAdmin={isAdmin} />)}
                  </div>
                )}
              </div>
            )}
